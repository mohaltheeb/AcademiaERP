		...
		if self.based_on == "Program":
				...
				if self.academic_program == "All Programs":
						...
						for program in program_specifications:
							...
							for child in program_specification_doc.table_ytno:
								...
								course_exists_in_course_study = frappe.db.exists(
									"Course Study", {"course_name": child_data["course_name"]}
								)
						...
						if self.specific_level:
							...
							for program in program_specifications:
								program_specification_doc = frappe.get_doc(
									"Program Specification", program.name
								)
								for child in program_specification_doc.table_ytno:
									child_data = child.as_dict()
									...
									if not course_exists_in_course_study:
										...
										child_table_data.append(child_data)
							filterd_data_by_specific_level = [
								d for d in child_table_data if d.get("study_level") == self.specific_level
							]
					...
					if self.specific_program:
						if self.level == "All Levels":
							...
							program_specification_doc = frappe.get_doc(
								"Program Specification", self.specific_program
							)
							for child in program_specification_doc.table_ytno:
								child_data = child.as_dict()
			...
			if self.student_batch:
				...
				if self.program_specification == "Finished":
					...
					program_specification_doc = frappe.get_doc(
						"Program Specification", self.program_specification
					)
					for child in program_specification_doc.table_ytno:
						child_data = child.as_dict()
